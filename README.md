...
Возможная поддержка IOS, MacOS, Linux. Приложение на данный момент на данных платформах не проверялось.
...
|-|-|-|-|
| Главный экран | Описание.<br>Часть с текстом | Описание.<br>Часть с численной<br>информацией | Полноэкранный режим |
...
* При нажатии на слайд отображается подробная информация (порода, альтернативное название породы, страна происхождения, детальное описание, информация о характере животного, вес и продолжительность жизни) и то же самое изображение кота.
* Для возможности корректного просмотра изображения реализован дополнительно полноэкранный режим. Для перехода в него необходимо нажать по изображению на экране с дополнительной информацией. Это позволит приблизить, отдалить изображение и свободно передвигаться жестами по нему.
* Дополнительная информация о коте может содержать ссылку на страницу википедии, которая кликабельна.
* Используется символичный индикатор загрузки в виде лапки кота при первой загрузке (на устройствах, не поддерживающих splash экран), а также для каждого изображения.
...
* Зафиксирована портретная ориентация для лучшего отображения. Приложение открывается и работает в полноэкранном режиме: скрыты навигационная и статусная панели, которые можно открыть привычным способом (с дальнейшим восстановлением к исходному состоянию).
...
Если вы используете [``installer``](https://github.com/azinhoxx/cat_tinder/tree/main/app/build/windows/installer), то будьте готовы к тому, что из самого установщика приложения не запустится после завершения. Это связано с проблемой сборщика и различия названия ``.exe`` (``PascalCase`` :vs: ``snake_case``). 
...
В директории [``apk/release``](https://github.com/azinhoxx/cat_tinder/tree/main/app/build/android/apk/release) также дополнительно находятся файлы формата APK под различные архитектуры: ``armeabi-v7a``, ``arm64-v8a``, ``x86_64``. Если вы уверены в архитектуре вашего устройства, то предпочтильнее использовать их, так как они весят меньше ввиду сборки под конкретную архитектуру.
...
Из-за ограничения CORS, в Web версии отсутствуют изображения. Более того, дополнительная проверка интернета отключается — считается, что подключение всегда есть.

В остальном приложение работает аналогично Windows, Android.

<u>На данный момент</u>:
1. Изображения котов на слайдах или в детальном описании могут отображаться некорректно — обрезано. Подробнее было описано в возможностях приложения. 
2. Поддерживается только англоязычная версия. Нет возможности выбора русского языка.

# APK

Скачать APK последней версии можно по следующей ссылке: [APK](https://minhaskamal.github.io/DownGit/#/home?url=https://github.com/azinhoxx/cat_tinder/blob/main/app/build/android/apk/release/app-release.apk).
