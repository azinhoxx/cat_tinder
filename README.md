...
Позволяет избавить пользователя от двух типов загрузки -- приложения и получения данных при старте приложения.
...
На данный момент поддерживаются только Android и Windows платформы.
...
На первом изображении — на Android поддерживается кастомный Splash-экран, который отображается во время первой **успешной** загрузки данных в приложении.
